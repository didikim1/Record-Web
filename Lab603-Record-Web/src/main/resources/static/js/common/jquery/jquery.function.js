jQuery.fun = {
	json_encode : function(data){
		...
		if(!$("#"+conf.id).length) $("body").append($("<div>").attr("id", conf.id));
		...
		this.layout({id:"fn_js_confirm",title:conf.title ? conf.title:'메세지',content:conf.content,width:conf.width,action:conf.action,closeAction:conf.closeAction, buttons:{}});
	},alert:function(conf){
		this.layout({id:conf.id,title:conf.title ? conf.title:'메세지',position:conf.position ? conf.position:'center',content:conf.content,width:conf.width ? conf.width:400 ,closeAction:conf.action,buttons:conf.buttons});
	},closeLayout:function(object){
		$(object).remove();
	},ajax:function(option){
		$.fun.layout({id:"ajax_loding",title:'Loading..',content:"<div style='text-align:center;padding:20px 0px;'>잠시만 기다려주세요...<div>",width:400,buttons:{}});
		$.ajax(option);
		...
		var returnInfo = {checked:0,firstChecked:null,lastChecked:null,firstNode:null,lastNode:null,checkedList:Array(),checkedListObject:Array()};
		var checkbox = document.getElementsByName(name);
...
$.ajaxSetup({
	...
	global:true,
	complete:function(){
		if($("#ajax_loding").length>0)
			$("#ajax_loding").dialog('destroy').remove();
	}
});
